...
export const CharacterAvatar: React.FC<CharacterAvatarProps> = ({ id, name, className = "w-16 h-16 sm:w-20 sm:h-20" }) => {
  switch (id) {
    case 'shoko':
      // 牧之原翔子 - 不裁切頭部，放大並聚焦於臉部
      return (
        <div className={`${className} rounded-2xl border-2 border-amber-400/90 shadow-md overflow-hidden bg-amber-950/20 shrink-0 relative flex items-center justify-center`}>
          <img
            src={shokoSmilingImg}
            alt={name}
            className="w-full h-full object-cover object-[center_10%] scale-135 origin-[50%_12%] hover:scale-145 transition-transform duration-300"
          />
        </div>
      );
