...
import { QuestionMemoryModal, QuestionHistoryItem } from './components/QuestionMemoryModal';
import { SpatiotemporalEnergyModal } from './components/SpatiotemporalEnergyModal';
...
import shokoServingMealImg from './assets/images/Shoko_tray.jpg';
import shokoFeedImg from './assets/images/Shoko_tray.jpg';
...
export default function App() {
  ...
  // Question Memory & Word Cloud State
  const [questionHistory, setQuestionHistory] = useState<QuestionHistoryItem[]>(() => {
    try {
      const saved = localStorage.getItem('mindful_latte_question_memory');
      return saved ? JSON.parse(saved) : [];
    } catch {
      return [];
    }
  });
  const [showQuestionMemoryModal, setShowQuestionMemoryModal] = useState<boolean>(false);

  // Spatiotemporal Energy Advice Modal State
  const [showEnergyAdviceModal, setShowEnergyAdviceModal] = useState<boolean>(false);
  const [hasShownEnergyAdvice, setHasShownEnergyAdvice] = useState<boolean>(false);

  const saveQuestionToHistory = (q: string, mode: string) => {
    if (!q || !q.trim()) return;
    const trimmed = q.trim();
    setQuestionHistory((prev) => {
      if (prev.length > 0 && prev[0].question === trimmed) return prev;
      ...
      try {
        localStorage.setItem('mindful_latte_question_memory', JSON.stringify(updated));
      } catch {
        // Fallback ignore
      }
      return updated;
  ...
  const handleClearQuestionHistory = () => {
    setQuestionHistory([]);
    try {
      localStorage.removeItem('mindful_latte_question_memory');
    } catch {
      // ignore
    }
  };

  const handleDeleteQuestionHistoryItem = (id: string) => {
    setQuestionHistory((prev) => {
  ...
  // Auto popup Spatiotemporal Energy Advice Modal upon flip completion in Divination mode
  useEffect(() => {
    if (
      readingMode === 'divination' &&
      matrixCards.length === 9 &&
      revealedCards.length === 9 &&
      revealedCards.every(Boolean) &&
      !hasShownEnergyAdvice &&
      !isDealing &&
      shufflingStage === 'done'
    ) {
      const timer = setTimeout(() => {
        setShowEnergyAdviceModal(true);
        setHasShownEnergyAdvice(true);
      }, 450);
      return () => clearTimeout(timer);
    }
  }, [readingMode, matrixCards, revealedCards, hasShownEnergyAdvice, isDealing, shufflingStage]);
  ...
  return (
    <div className="min-h-screen bg-[#F5EBE6] text-[#4A3E3D] font-serif transition-colors duration-500 selection:bg-[#E4D5C7] selection:text-[#4A3E3D] pb-16 relative overflow-hidden">
      ...
      <header className="max-w-6xl mx-auto px-3 sm:px-4 pt-5 sm:pt-10 pb-4 sm:pb-8 text-center border-b border-[#E4D5C7] mb-4 sm:mb-8">
        ...
        {/* QUESTION MEMORY & WORD CLOUD CONTROL BUTTON */}
        <div className="flex justify-center items-center mt-3 sm:mt-4">
          <button
            type="button"
            onClick={() => setShowQuestionMemoryModal(true)}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs sm:text-sm font-serif font-extrabold bg-[#FAF4F0] text-[#4A3E3D] border-2 border-[#A87C66]/60 hover:bg-[#A87C66] hover:text-white transition-all shadow-md hover:shadow-lg cursor-pointer group"
          >
            <Brain className="w-4 h-4 text-[#A87C66] group-hover:text-amber-200 transition-colors" />
            <span>問題記憶與心靈文字雲</span>
            {questionHistory.length > 0 && (
              <span className="text-[10px] bg-[#A87C66] group-hover:bg-amber-300 group-hover:text-amber-950 text-white font-mono font-bold px-2 py-0.5 rounded-full transition-colors">
                    ...
                    {hasHandedMenu && (
                      ...
                      >
                        ...
                        <div className="pt-2 sm:pt-3 border-t-2 border-dashed border-[#E4D5C7] text-center">
                          <OrderBell
                            onRingBell={() => {
                              saveQuestionToHistory(userQuestion, readingMode);
                    ...
                    <div id="nine-grid-cards" className="max-w-4xl mx-auto space-y-3 sm:space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 px-1 sm:px-2">
                        ...
                        <div className="flex flex-wrap gap-1.5 sm:gap-2 text-[11px] sm:text-xs">
                          {readingMode === 'divination' && matrixCards.length === 9 && (
                            <button 
                              type="button"
                              onClick={() => setShowEnergyAdviceModal(true)}
                              className="bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white px-2.5 py-1 rounded-lg transition-all flex items-center gap-1 shadow-xs cursor-pointer font-bold"
                            >
                          ...
                          <button 
                            ...
                            className="bg-[#A87C66] text-[#F5EBE6] px-2.5 py-1 rounded-lg hover:bg-[#966b56] transition-all flex items-center gap-1 shadow-xs cursor-pointer"
                          >
                      ...
                      {(() => {
                        ...
                        let type: 'yesno' | 'rightwrong' | 'should' | 'choice' | 'divination' = 'yesno';
                        
                        if (readingMode === 'divination') {
                          type = 'divination';
                        } else if (readingMode === 'decision' || (quickAnswer.optionBreakdowns && quickAnswer.optionBreakdowns.length > 0)) {
                          type = 'choice';
                        ...
                        if (type === 'divination') {
                          answerLabel = isPositive ? '順流吉兆' : '審慎應對';
                          explanation = quickAnswer.directAnswerSummary;
                          stampColor = isPositive ? 'text-amber-800 border-amber-600 bg-amber-50' : 'text-[#4A3E3D] border-[#7A6A63] bg-[#FAF4F0]';
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="space-y-4">
                              ...
                              {readingMode === 'decision' && quickAnswer.optionBreakdowns && quickAnswer.optionBreakdowns.length > 0 && (
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-[11px] text-[#A87C66] font-serif">
                                      角色意義：{GRID_POSITIONS[activeZoomCardIndex].role || GRID_POSITIONS[activeZoomCardIndex].description}
                                    </p>
                                  </div>
                                </div>
                                <button
                                      ...
                                      <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
                                        <div className="w-44 h-64 sm:w-52 sm:h-76 flex-shrink-0 hover:shadow-2xl hover:scale-[1.01] transition-all duration-300 relative">
                                          <div className="w-full h-full bg-gradient-to-b from-white via-[#FFFDF9] to-[#F8F3ED] border-2 border-[#A87C66] rounded-xl p-2.5 sm:p-3.5 flex flex-col justify-between relative select-none">
                                            ...
                                            {/* Center Area: Interpretation Text */}
                                            <div className="flex-1 flex flex-col justify-center my-1.5 text-center overflow-y-auto px-1">
                                              <p className="text-xs sm:text-sm text-[#4A3E3D] font-serif font-medium leading-relaxed max-h-[160px] sm:max-h-[220px] overflow-y-auto scrollbar-thin">
                                                {getCardInterpretation(cardObj, activeZoomCardIndex).text}
                                              </p>
                                            </div>
                                  ...
                                  <div className="flex-1 space-y-3 flex flex-col justify-between h-full w-full">
                                    ...
                                    <div className="bg-amber-50/90 border border-amber-300 rounded-xl p-3.5 space-y-1 text-xs text-[#4A3E3D] font-serif flex-shrink-0">
                                      ...
                                      <p className="leading-relaxed text-xs text-[#5C4D4B]">
                                        {getShokoMagicDialogue(matrixCards, userQuestion)}
                                      </p>
                                    </div>
                                  </div>
                                </div>
                ...
                {isProcessingLuck ? (
                  ...
                  <div className="py-2 space-y-6 text-center">
                    ...
                    <div className="relative flex flex-col items-center">
                      ...
                      {/* Photo Frame with Shoko Holding Meal Tray with Overlay Dialogue */}
                      <div className="relative w-64 h-80 sm:w-72 sm:h-96 rounded-3xl p-2 bg-gradient-to-b from-[#FFFDF9] via-[#F5EBE6] to-[#C2AF9E] shadow-2xl border-4 border-[#8C5C42] overflow-hidden group">
                        <div className="relative w-full h-full rounded-2xl overflow-hidden bg-gradient-to-b from-amber-100 to-amber-200/50 flex flex-col justify-end">
                          <img
                            src={shokoServingMealImg}
                            alt="翔子親自端餐，將托盤抱在胸前微笑"
                            referrerPolicy="no-referrer"
                          ...
                          {/* Dialogue Text Overlaid at the Very Bottom of Image */}
                          <div className="relative z-10 p-2.5 sm:p-3 m-2 bg-black/65 backdrop-blur-md border border-amber-300/50 rounded-xl text-center text-amber-50 font-serif shadow-lg space-y-1">
                            <div className="font-extrabold text-amber-200 text-xs sm:text-sm leading-snug">
                              翔子：「久等囉！為您端上『{unlockedLuckBlessing.name}』」
                            </div>
                            <div className="text-amber-100/90 font-medium text-[10px] sm:text-xs">
                              還有附上了轉運御神紙籤喔 💓，請慢用～
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3.5 items-center sm:items-start">
                        ...
                        <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl border-2 border-amber-400 overflow-hidden shadow-md shrink-0 bg-amber-950 relative">
                          <img
                            src={shokoSmilingImg}
                            alt="翔子特調"
                            className="w-full h-full object-cover object-[center_10%] scale-135 origin-[50%_12%]"
                          />
      ...
      {/* QUESTION MEMORY & WORD CLOUD MODAL */}
      <QuestionMemoryModal
        isOpen={showQuestionMemoryModal}
        onClose={() => setShowQuestionMemoryModal(false)}
        history={questionHistory}
        onSelectQuestion={(q) => {
          setUserQuestion(q);
          setHasHandedMenu(true);
          setShowQuestionMemoryModal(false);
        }}
        onClearHistory={handleClearQuestionHistory}
        onDeleteHistoryItem={handleDeleteQuestionHistoryItem}
      />

      {/* SPATIOTEMPORAL ENERGY ADVICE MODAL */}
      <SpatiotemporalEnergyModal
        isOpen={showEnergyAdviceModal}
        onClose={() => setShowEnergyAdviceModal(false)}
        cards={matrixCards}
        userQuestion={userQuestion}
      />
