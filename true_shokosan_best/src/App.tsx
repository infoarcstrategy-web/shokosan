                  ...
                  <div className="space-y-8">
                    ...
                    <div id="nine-grid-cards" className="max-w-4xl mx-auto space-y-3 sm:space-y-4">
                      ...
                      {(() => {
                        ...
                        if (readingMode === 'decision' || q.includes('還是') || q.includes('選擇') || q.includes('哪個')) {
                          type = 'choice';
                        ...
                        if (type === 'choice') {
                          const optA = (extractedOptions && extractedOptions[0]) ? extractedOptions[0] : '選項 A';
                          const optB = (extractedOptions && extractedOptions[1]) ? extractedOptions[1] : '選項 B';
                          answerLabel = isPositive ? optA : optB;
                          explanation = isPositive 
                            ? `在多選項契合度評估中，當前【${optA}】的顯化機率為 ${probability}%。這代表【${optA}】的時空維度與您的心靈磁場最為共振，是更適合當下顯化的路徑。` 
                            : `在多選項契合度評估中，【${optA}】的顯化機率僅為 ${probability}%，此時更偏向【${optB}】（或者暫緩決定）。【${optA}】目前面臨較多隱形制約，建議多加斟酌、理性重整。`;
